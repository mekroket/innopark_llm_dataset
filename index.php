...
<body class="overflow-hidden">
    <div class="flex h-screen">
        ...
        <div class="w-72 glass hidden md:flex flex-col p-4 border-r border-slate-800">
            ...
            <button onclick="newChat()" class="w-full py-3 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 transition-all border border-slate-700 flex items-center gap-3 mb-6 shadow-lg active:scale-95">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
                Yeni Sohbet
            </button>

            <div class="flex-1 overflow-y-auto space-y-6 scrollbar-hide pb-4">
                
                <!-- Geçmiş Sohbetler -->
                <div>
                    <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 px-2">Geçmiş Sohbetler</div>
                ...
                <!-- Hızlı Sorgular -->
                <div>
                    <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 px-2 mt-4">Hızlı Sorgular</div>
    ...
    <script>
        ...
        // Sol menüye geçmiş sohbetleri çiz
        function renderHistory() {
            chatHistoryList.innerHTML = '';
            const sortedChats = [...chats].sort((a, b) => b.id - a.id);
            sortedChats.forEach(chat => {
                const btn = document.createElement('button');
                btn.className = `w-full text-left p-2 rounded-lg hover:bg-slate-700 transition-colors text-sm truncate flex items-center gap-2 ${chat.id === currentChatId ? 'bg-slate-800 text-emerald-400' : 'text-slate-400'}`;
                btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> <span class="truncate">${chat.title}</span>`;
        ...
        // Eski sohbete tıklandığında yükle
        function loadChat(id) {
            ...
            chat.messages.forEach(msg => {
                appendMessage(msg.role, msg.text, false); // Eski sohbetlerde animasyon yapma
            });
            renderHistory();
        }
        ...
        async function handleSend() {
            ...
            // Eğer sohbet yeni başladıysa ID ata ve başlık oluştur
            if (!currentChatId) {
            ...
            try {
                ...
                if (data.error) {
                    if (data.auth_required) {
                        ...
                        appendMessage('error', `❌ Bir hata oluştu: ${data.error}`, false);
                    }
                } else {
                    appendMessage('ai', data.response, true); // Sadece yeni cevapları animasyonlu yazdır
        ...
        function appendAuthCard() {
            ...
            msgDiv.className = `flex justify-center animate__animated animate__fadeInUp mb-4 w-full`;
            
            msgDiv.innerHTML = `
                <div class="bg-slate-800 border border-red-500/50 p-6 rounded-xl text-center w-full max-w-lg shadow-2xl">
                    <div class="flex justify-center mb-4">
                        <div class="w-12 h-12 bg-red-500/20 rounded-full flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        </div>
                    </div>
                    <h3 class="text-xl font-bold text-white mb-2">Oturum Süresi Doldu</h3>
                    <p class="text-slate-400 mb-6 text-sm">NotebookLM bağlantısı güvenlik nedeniyle kapatıldı. Lütfen Google hesabınızla yeniden giriş yapın.</p>
                    <button onclick="triggerLogin(this)" class="w-full bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg font-semibold transition-all flex justify-center items-center gap-2 shadow-lg shadow-red-500/20 active:scale-95">
                    ...
                    <p class="mt-3 text-xs text-slate-500 login-status"></p>
                </div>
            `;
        ...
        async function triggerLogin(btn) {
            btn.innerHTML = '<span class="animate-pulse">Chrome Açılıyor... Bekleyin</span>';
            ...
            try {
                ...
                const data = await response.json();
                if (data.success) {
                    btn.className = "w-full bg-emerald-500 text-white py-3 rounded-lg font-semibold cursor-default";
                    btn.innerHTML = 'Chrome Üzerinden Onay Verin ✓';
                    statusText.textContent = "Google giriş işlemi tamamlandıktan sonra sormaya devam edebilirsiniz.";
                }
            } catch (e) {
        ...
        function appendMessage(role, text, animate = true) {
            ...
            if (role === 'ai') {
                ...
                if (animate) {
                    ...
                    // Eski sohbetleri yüklerken bekleme olmasın
                    innerDiv.innerHTML = marked.parse(text); 
                    scrollToBottom();
                }
                return;
